...
transData1 <- preProcess(data1,
                         method = c("BoxCox", "pca", "scale", "center"))
...
summary(svmFit2)
```
The out-of-sample accuracy appears to be good across all the folds for both model fitted: precisely, the total accuracy is `r svmFit1$tot.accuracy` for the first model, and `r svmFit2$tot.accuracy` for the second one; while the time to compute is slightly less (`r total.time1` minutes) for the first model, compared to the second one (`r total.time2` minutes).
...
table(preds1, preds2)
```
The two models predict almost the same classes for the 20 testing observations.  
In conclusion, they appear identical if we look at their total accuracy in predicting the outcomes.
